...
from serializers.orders import BookingResponseModel, CreateBookingRequest, BookingResponse

# TESTS WITHOUT TOKEN REQUIRED
...
@pytest.mark.parametrize('headers, request_body, expected_status', [
    ({"Content-Type": "application/json", "Accept": "application/json"},
     {"firstname": "Jim", "lastname": "Brown", "totalprice": 111, "depositpaid": True,
        "bookingdates": {"checkin": "2018-01-01", "checkout": "2019-01-01"}, "additionalneeds": "Breakfast"}, 200),     # Valid data positive test
    ({"Content-Type": "text/plain", "Accept": "text/plain"},
     {"firstname": "Jim", "lastname": "Brown", "totalprice": 111, "depositpaid": True,
      "bookingdates": {"checkin": "2018-01-01", "checkout": "2019-01-01"}, "additionalneeds": "Breakfast"}, 415),       # Invalid headers negative test
    (None, {"firstname": "Jim", "lastname": "Brown", "totalprice": 111, "depositpaid": True,
        "bookingdates": {"checkin": "2018-01-01", "checkout": "2019-01-01"}, "additionalneeds": "Breakfast"}, 200)])    # No headers negative test
def test_create_booking(headers, request_body, expected_status):
    ...
    if expected_status == 200:
        ...
        assert booking.bookingdates.model_dump() == request_data.bookingdates.model_dump(), "Invalid bookingdates in the response"
